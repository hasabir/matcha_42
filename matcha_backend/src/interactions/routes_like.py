...
# src/interactions/routes_like_dislike.py  (or wherever your interactions_bp is defined)
import os
import sys
import logging
...
def like_dislike():
    ...
    Server decides to like or dislike depending on current state.
    """
    try:
        requested_data = request.json or {}
        liked_user = requested_data.get("liked_user")
        if not liked_user:
            return jsonify({"error": "Request must include 'liked_user'"}), 400
        ...
        my_profile = profile_crud.get_profile_by_user_id(g.user_id)
        if not my_profile or not my_profile.get("profile_picture"):
            return jsonify({"error": "You need a profile picture to complete this action"}), 409

        user_crud = User(pool)
        my_username = user_crud.get_user_by('id', g.user_id, 'username')
        if liked_user == my_username:
            return jsonify({"error": "You cannot like or dislike yourself"}), 409

        target_user = user_crud.get_user_by_username(username=liked_user)
        if not target_user:
            return jsonify({"error": "liked user does not exist"}), 409

        target_profile = profile_crud.get_profile_by_user_id(target_user["id"])
        if not target_profile:
            return jsonify({"error": "The user you are trying to like/dislike does not have a profile"}), 409

        interactions_crud = Interactions(pool, g.user_id, target_user["id"])
        ...
        return jsonify({
            "status": "ok",
            "message": f"user has {action} {liked_user}",
            "new_fame_rating": new_rating
        }), 201

    except Exception as e:
        logger.exception("like_dislike failed")
        return jsonify({"error": str(e)}), 400


@interactions_bp.route("/get_users/<interaction_type>", methods=["GET", "OPTIONS"])
@auth_guard
def get_user_likes(interaction_type):
    """Return usernames you liked or users who liked you."""
    if request.method == "OPTIONS":
        return ("", 204)

    try:
        if interaction_type not in ["liked", "likers"]:
            return jsonify({"error": "interaction_type must be 'liked' or 'likers'"}), 400
        ...
        logger.exception("get_user_likes failed")
        return jsonify({"error": str(e)}), 400
...
def is_matched():
    ...
    Return whether current user is matched with other_user.
    """
    try:
        requested_data = request.json or {}
        if "other_user" not in requested_data:
            return jsonify({"error": "Request must include 'other_user'"}), 400
        ...
        my_username = user_crud.get_user_by('id', g.user_id, 'username')
        if requested_data["other_user"] == my_username:
            return jsonify({"error": "You cannot check match status with yourself"}), 409
        ...
        matching_crud = Matching(connection_pool)
        matched_users_ids = matching_crud.get_matched_users(g.user_id)
        is_matched = bool(matched_users_ids and other_user_data["id"] in matched_users_ids)

...
@auth_guard
def set_location():
    """JSON: { latitude: float, longitude: float, city?: str, country?: str, accuracy?: int }"""
    try:
        data = request.get_json(force=True) or {}
        if "latitude" not in data or "longitude" not in data:
            return jsonify({"error": "Missing required field: latitude/longitude"}), 400
        ...
        loc.set_user_location(
            ...
            accuracy=data.get("accuracy") if data.get("accuracy") is not None else 50,
        )
        return jsonify({"status": "ok"}), 200
    except Exception as e:
...
@auth_guard
def get_nearby_users():
    """Query: max_distance (km, default 100)"""
    try:
        ...
        loc = Location(pool)
        users = loc.find_nearby_users(g.user_id, max_distance)
        return jsonify({"status": "ok", "nearby_users": users, "count": len(users)}), 200
    except Exception as e:
        logger.exception("get_nearby_users failed")
        return jsonify({"error": "Internal server error"}), 500
...
@auth_guard
def get_user_location(username):
    """Get location for <username> (or 'me'). Enforces blocking rules."""
    try:
        pool = current_app.config.get("CONNECTION_POOL")
        if not pool:
            return jsonify({"error": "Database connection pool is not available"}), 500
        ...
        location = loc.get_user_location(user_id)
        if not location:
            return jsonify({"error": "Location data not found for the user"}), 404

        return jsonify({"status": "ok", "location": location}), 200
    except Exception as e:
        logger.exception("get_user_location failed")
        return jsonify({"error": "Internal server error"}), 500

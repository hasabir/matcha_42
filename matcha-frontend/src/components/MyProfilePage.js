...
const MyProfilePage = () => {
  ...
  const getImageId = (img) => {
    if (!img) return null;
    return typeof img === 'object' ? img.image_id : null;
  };
  ...
  const uniqueAdditionalImages = [...new Set((profile?.images || []).map(img => {
    const url = getImageUrl(img);
    return url !== profilePicture ? img : null;
  }))].filter(Boolean);
  ...
  const handleDeletePhoto = async (photo) => {
    ...
    const photoId = getImageId(photo);
    
    console.log("🗑️ Delete photo requested for:", photo, "ID:", photoId, "URL:", photoUrl);
    ...
    try {
      ...
      // Prefer image_id if available, fallback to image_path
      ...
      setProfile(prev => {
        ...
        const updatedProfilePic = prev.profile_picture === photoUrl ? null : prev.profile_picture;
        return {
          ...prev,
              ...
              <div className="photo-grid">
                {allImages.map((img, index) => (
                  <div key={index} className="photo-item">
                    <img 
                      src={toAbsoluteUrl(getImageUrl(img))} 
                      alt={`Upload ${index + 1}`}
                      onError={onImgError}
                    />

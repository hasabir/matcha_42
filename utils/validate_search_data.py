...
def validate_search_data(data):
    ...
    optional_fields = ['location', 'coordinates', 'distance']    
    if not data:
        return False, "No data provided."
    
    incorrect_fields = [field for field in data if field not in required_fields or\
                                field not in optional_fields]
    missing_fields = [field for field in required_fields if field not in data]

    if incorrect_fields:
        return False, f"Unexpected fields: {', '.join(incorrect_fields)}"
    if missing_fields:
    ...
    if (not isinstance(age_range, dict) or len(age_range) != 2 or
        ...
        False, "Invalid age_range. It should be a dictionary with two keys: \
    ...
    if 'coordinates' in data:
        ...
        if not isinstance(location, dict) or len(location) != 2 or \
            ...
            return False, "Invalid location. It should be a non-empty string."
